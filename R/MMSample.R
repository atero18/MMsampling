...
MMSample$methods(
  initialize = function(Z, pi, I, modes, Yobs, phi = rep(1/2, length(Yobs)))
  {
    .self$Z <- as.data.frame(Z)
    .self$pi <- pi
    .self$I <- set_I(I, N = length(pi))

    modesExt <- modes
    modesExt[is.na(modesExt)] <- "nr"
    .self$modes <- as.factor(modesExt)
