                            ...
                            nCores = detectCores() - 1L)
{
  ...
  monoSim <- function(...)
  {
    ...
    meanPhi1TW <- totPhi1TW / sum(phi[maskInt] * trueWeights[maskInt])
    ...
    meanPhi1EW <- totPhi1EW / sum(phi[maskInt] * estimWeights[maskInt])
    ...
    meanPhi2TW <- totPhi2TW / sum(phi[maskTel] * trueWeights[maskTel])
    # meanPhi2TW <- NA_real_
    ...
    meanPhi2EW <- totPhi2EW / sum(phi[maskTel] * estimWeights[maskTel])
    # meanPhi2EW <- NA_real_
...
paramVarRatioSigma <- expand.grid(ratio_delta = ratioDeltas, ratio_n = nMin,
                                  sigma1 = sigmavec[1L],
                                  ratiosigma = ratiosigmavec,
                                  rho = rhoRef)
  
parameters <- 
  rbind(paramVarn, paramVarRho, paramVarSigma) %>% 
  mutate(sigma1 = sigma, ratiosigma = 1.0, sigma2 = sigma) %>% 
  select(-sigma) %>% 
  ...
  mutate(sigma2 = sigma1 * ratiosigma, .after = "ratiosigma") %>% 
  distinct()
...
=======
                             seed = NULL,
                             nCores = 30L)
>>>>>>> 783188cef561d03c1a61458babf773cd389582ac
...
resSimBiasDetection <-
  resSimBiasDetection %>%
  arrange(n, sigma1, sigma2, ratio_delta)
```
