...
simulationUncounf <- 
           ...
           n = ceiling(N / 4L))
  {
    ...
    monoSim <- function(...)
    {
      ...
      trueWeightsBenchmark <- sum(trueWeights[maskTel] * Ytel[maskTel])
    ...
    if (fixZ)
    {
      ...
      sdBenchmark <- estim_var_HT_seq_2(expY2 = Z %*% betaTel, 
      ...
      results <- results %>% 
        mutate(sdBenchmark = sdBenchmark)
    }
...
results %>% 
  ...
  summarise(#reldiffTotalTrueMean = round(mean(diffTotalYtelTrueMP / abs(expectedTotalYtel)) , 4L), 
            ...
            relSdBenchmark = mean(sdBenchmark),
            test = sd(diffTotalYtelEstMP)) %>% 
  ungroup() %>% 
  arrange(relRMSEEst)
```
