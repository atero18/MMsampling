                            ...
                            seed = 123L)
{
  ...
  monoSim <- function(...)
  {
    ...
    expCtrdPhi1 <- expPhi1 / sum(phi) - mean(expYints)
    totCtrdPhi1TW <- 
      sum(phi[maskInt] * trueWeights[maskInt] * (Yobs[maskInt] - mean(Yints))) /
      sum(phi[maskInt] * trueWeights[maskInt])
    
    apprVarCtrdPhi1TW <- 
      estim_var_centered_phi1(Yobs,
                                modes, 
                                I,
                                piMat,
                                pq1Mat,
    ...
    apprVarPhi2EW <- NA_real_
      ...
      #  equal to phi_k y_2k / sum of the phi_k on U - average on U of the y_2l
      #  (the last term is not a statistic)
    expCtrdPhi2 <- expPhi2 / sum(phi) - mean(expYtels)
    totCtrdPhi2TW <- 
      sum(phi[maskTel] * trueWeights[maskTel] * (Yobs[maskTel] - mean(Ytels))) /
      sum(phi[maskTel] * trueWeights[maskTel])
    
    apprVarCtrdPhi2TW <-
      estim_var_centered_phi2(Yobs,
                              modes,
                              I,
                              piMat,
                              pq1Mat,
    ...
    apprVarCtrdPhi2EW <- NA_real_
    ## NOTE : Attention calcul probas estimées ici, précédemment pris sans
    ## conditionnement
    tibble(estimand = rep(c("phi1", "centered phi1", 
                            "phi2", "centered phi2"), each = 2L), 
           expEstimand = rep(c(expPhi1, expCtrdPhi1, expPhi2, expCtrdPhi2), 
                             each = 2L),
           estimator = c("totPhi1TW", "totPhi1EW", 
                         "totCtrdPhi1TW", "totCtrdPhi1EW", 
                         "totPhi2TW", "totPhi2EW",
                         "totCtrdPhi2TW", "totCtrdPhi2EW"), 
           estimation = c(totPhi1TW, totPhi1EW, 
                          totCtrdPhi1TW, totCtrdPhi1EW, 
                          totPhi2TW, totPhi2EW,
                          totCtrdPhi2TW, totCtrdPhi2EW), 
           apprVar = c(apprVarPhi1TW, apprVarPhi1EW, 
                       apprVarCtrdPhi1TW, apprVarCtrdPhi1EW,
                       apprVarPhi2TW, apprVarPhi2EW,
                       apprVarCtrdPhi2TW, apprVarCtrdPhi2EW)) %>% 
...
```
```{r}
mean(compareTphi$estimApprVarPhi1[-seq_len(Kbis / 2.0)]) / varPhi1
```

```{r}
var(compareTphi$totCtrdPhi1EW[seq_len(Kbis / 2.0)])
```
...
```{r}
mean(compareTphi$estimApprVarCenteredInt[-seq_len(Kbis / 2.0)]) / var(compareTphi$totCtrdPhi1EW[seq_len(Kbis / 2.0)])
```
...
{
  data1 <- resSimulations %>% filter(delta == delt, 
                                     estimand == "centered phi1", 
                                     weights == "true")
  
  data2 <- resSimulations %>% filter(delta == delt, 
                                     estimand == "centered phi2", 
                                     weights == "true")
