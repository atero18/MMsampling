                            ...
                            seed = 123L)
{
  ...
  monoSim <- function(...)
  {
    ...
    # Estimation of mode selection probabilities
    estimProbsData <- estim_response_prob_sequential(I, pi, Z,
                                                 modes, 
                                                 c("int", "tel"),
                                                 chosenOnly = FALSE)
    
    estimProbsSelect <- 
      get_value_by_mode(estimProbsData$unconditional, modes)
    estimWeights <- (pi * estimProbsSelect)^-1L
    ...
    estimpq1Mat <- 
      estimProbsData$conditional[, "int"] %*% 
      t(estimProbsData$conditional[, "int"])
    
    diag(estimpq1Mat) <- estimProbsData$conditional[, "int"]
    ...
    # apprVarCtrdPhi2TW <- 
    #   estim_var_centered_phitel(Yobs,
    #                             modes, 
    #                             I,
    #                             piMat,
    #                             pq1Mat,
...
compareTphi %>% 
  ...
  summarize(expEstimad = mean(expEstimand),
