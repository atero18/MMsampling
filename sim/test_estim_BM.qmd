                        ...
                        deltaLaw = "gaussian", ...)
{
  ...
  results <- results %>% 
    ...
    add_column(.after = "deltaLaw", BMtype = factor(ifelse(constBias, "constant", "variable")))
...
results %>% 
  ...
  group_by(BMtype, sampling, YtelLaw, deltaLaw, estimator, probSelect, invMatrix) %>% 
  summarise(reldiffMean = round(mean(diff / abs(expectedBias)) , 4L), 
            relRMSE = round(sqrt(mean(diff^2L / expectedBias^2L)), 4L)) %>% 
  ungroup() %>% 
  group_by(BMtype, sampling, YtelLaw, deltaLaw) %>% 
  # arrange(relRMSE) %>% 
  slice_min(abs(reldiffMean))
```
```{r}
results %>% 
  #filter(BMtype == "constant") %>% 
  ...
  summarise(reldiffMean = round(mean(diff / abs(expectedBias)) , 4L), 
            relRMSE = round(sqrt(mean(diff^2L / expectedBias^2L)), 4L)) %>% 
  ungroup() %>% 
  filter(YtelLaw == "gaussian", deltaLaw == "gaussian", BMtype == "constant") %>% 
  group_by(BMtype, sampling) %>% 
  arrange(relRMSE, .by_group = TRUE)
```
