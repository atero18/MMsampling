...
Creating parameters for two modes : "m1" and "m2". `delta` is defined such that the average measure effect is equal to 1. It depends on `X`.
...
pInt <- gen_selection_probabilities(Z, alpha1)

alpha2 <- c(0.3, 0.02, -0.5)
pTel <- gen_selection_probabilities(Z, alpha2)
```
                            ...
                            nCores = detectCores() - 1L)
{
  ...
  pInt <- expit(Z %*% alpha1) %>% as.numeric()
  pq1Mat <- pi_to_pi2(pInt)
  
  pTel <- expit(Z %*% alpha2) %>% as.numeric()
  ...
  expY1 <- dataExpY$expY1
  sd1s <- dataExpY$sd1s
  
  expY2 <- dataExpY$expY2
  sd2s <- dataExpY$sd2s
  ...
  monoSim <- function(...)
  {
    ...
    dataModes <-  gen_choice_bimode(I, pInt, pTel, Z)
    ...
    estimpq1 <- estimProbsData$conditional[, "m1"]
    # estimpq1Mat <- pi_to_pi2(estimpq1)
    estimpq1MatSr <- pi_to_pi2(estimpq1[mask1])
    estimpq1MatSmr <- pi_to_pi2(estimpq1[mask2])
    
     # Total of phi_k y_1k with true probabilities
    expPhi1 <- sum(phi * expY1)
    ...
    # Estimations with Y2
      ...
      # Estimation of the internet design covariance matrix
    estimpq2 <-  estimProbsData$conditional[, "m2"]
    # estimpq2Mat <- pi_to_pi2(estimpq2)
    estimpq2MatSmr <- pi_to_pi2(estimpq2[mask2])
...
paramVarRho <- 
  expand.grid(ratio_delta = ratioDeltas, ratio_n = nMin, sd = sdvec[1L], 
              rho = rhovec)
paramVarSD <- 
...
paramVarRatioSD <- expand.grid(ratio_delta = ratioDeltas, ratio_n = nMin,
                               sd1 = sdvec[1L],
                               ratiosd = ratiosdvec,
                               rho = rhoRef)
...
rm(paramVarn, paramVarRho, paramVarSD, paramVarRatioSigma)
...
for (k in seq_len(nrow(parameters)))
{
  ...
  beta1Temp <- beta2
  beta1Temp[1L] <- beta1Temp[1L] + delta
...
resSimBiasDetection %>% 
  filter(delta == 0.0,
         ...
         weights == "estimated") %>% 
  group_by(n, sd1, rho) %>% 
  summarise(shapiro = shapiro.test(estimation))
...
temp <- temp %>% 
  ...
  group_by(estimand, weights, n, sigma1, sigma2, rho) %>% 
  summarize(M = n(),
...
expY1_list <- list("0" = expY2)
for (delta in unique(temp$delta))
  expY1_list[[as.character(delta)]] <- expY2 + delta
...
for (k in seq_len(nrow(temp) / 2L))
{
  ...
  n <- temp[k, "n"]
  vartotphi1TW <- true_variance_tephi1(expY1_list[[as.character(delta)]], 
                                       pi, covarSRS, 
                                       pInt,
                                       phi, 
                                       sd1 = sigma)
...
simulationUncounf <- 
           ...
           cluster = NULL)
  {
    ...
    pInt <- expit(X %*% alpha1) %>% as.numeric()
    pq1Mat <- pInt %*% t(pInt)
    diag(pq1Mat) <- pInt
    
    pTel <- expit(X %*% alpha2) %>% as.numeric()
    pq2Mat <- pTel %*% t(pTel)
    diag(pq2Mat) <- pTel
    ...
    # {
    #   expY1 <- as.vector(X %*% beta1)
    #   quantilesInt <- quantile(expY1, probs = c(1.0 / 3.0, 2.0 / 3.0))
    #   
    # 
    #   sd1 <- as.numeric(sd1)
    ...
    #   expY1[expY1 > quantilesInt[1L]] <- 
    #     expY1[expY1 > quantilesInt[1L]] + 1.0
    #   
    #   # After the second quantile of 3rd degree we add one more
    #   expY1[expY1 > quantilesInt[2L]] <- 
    #     expY1[expY1 > quantilesInt[2L]] + 1.0
    # }
    
    expTotYtel <- sum(expY2)
    expTotYint <- sum(expY1)
    
    clusterExport(cluster,
                  varlist = c("expY2", "expTotYtel",
                              "expY1", "expTotYint",
                              "sd1s", "sd2s", "rho"),
                  envir = environment())
    ...
    monoSim <- function(...)
    {
      ...
      # if (Y2Law == "gaussian")
      # {
      #   Ytels <- expY2 + rnorm(n = N, sd = sd2s)
      # }
      #   
      # 
      # else if (Y2Law == "exponential")
      #   Ytels <- rexp(n = N, rate = expY2^-1L)
      # 
      # expTotYtel <- sum(expY2)
      # trueYtel <- sum(Ytels)
      # 
      # if (Y1Law %in% c("gaussian", "split3"))
      #   Yints <- expY1 + rnorm(n = N, sd = sd1s)
      # 
      # else if (Y1Law == "exponential")
      #   Yints <- rexp(n = N, rate = expY1^-1L)
      ...
      expPhiBias <- crossprod(phi, expY1 - expY2) %>% as.vector()
    ...
    sdTWBenchmark <- var_HT_seq_phi2(expY2 = expY2, 
                                     covarY2 = covarYtel,
                                     piMat = piMat,
                                     pq1Mat = pq1Mat,
                                     pq2Mat = pq2Mat,
