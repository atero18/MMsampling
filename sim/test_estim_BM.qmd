...
If `correctAlpha` is true then the size of the intervals are changed in the order to have a first type error equal to $\alpha$.
...
absRes %>% 
  ...
  filter(weights == "true", correctAlpha == "corrected") %>% 
  ggplot() +
...
absRes %>% 
  mutate(sigma = as.factor(sigma)) %>% 
  filter(weights == "estimated", correctAlpha == "corrected") %>% 
  ggplot() +
  geom_point(aes(x = ratio_delta,
                 y = percSuperpositionsAV,
                 colour = sigma)) +
...
absRes %>% 
  mutate(sigma = as.factor(sigma)) %>% 
  filter(weights == "estimated", sigma == 1.0) %>% 
  ggplot() +
  geom_point(aes(x = ratio_delta,
                 y = percSuperpositionsAV,
                 colour = correctAlpha)) +
  # geom_smooth(aes(x = ratio_delta,
  #                y = percSuperpositionsAV, colour = sigma, n = 3)) +
  ggtitle("Impact of the coverage rate correction")
```
