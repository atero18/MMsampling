           ...
           n = ceiling(N / 4L))
  {
    ...
    monoSim <- function(...)
    {
      ...
      partialResults <- data.frame(parameter = character(nbExperiments),
                                   ...
                                   expSD = rep(NA_real_, nbExperiments))
      ...
      partialResults[row,] <- 
        c(parameter = "HT", trueParameter = FALSE,
          ...
          calculTotal = "full", estTotPhiBias = resEval, expSD = expSD)
      ...
      partialResults$expSD <- as.numeric(partialResults$expSD)
...
parameters <- expand_grid(constBias = c(TRUE, FALSE), 
  ...
  filter(YintLaw == "gaussian", YtelLaw == "gaussian")
...
results %>% 
  ...
  summarise(#reldiffTotalTrueMean = round(mean(diffTotalYtelTrueMP / abs(expectedTotalYtel)) , 4L), 
            ...
            relExpSD = round(relExpSD = mean(expSD / expectedTotalYtel), 4L),
