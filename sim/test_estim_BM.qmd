...
```{r}
betaTel <- c(4.0, -10.0, 10.0)
print(betaTel)
delta <- c(1.0, -2.0 * mean(Z[, "age"])^-1L, 2 * N / sum(Z[, "sex"]))
                        ...
                        ...)
{
  ...
  {
    betaIntBis <- betaTel
    betaIntBis[1L] <- betaTel[1L] + 1L
  }
  else
    betaIntBis <- betaInt

  if (YintLaw == "gaussian")
    Yint <- Z %*% betaIntBis + rnorm(N, 0.0, 25.0)
  ...
  benchmark <- sum(estimWeights[modes == "tel"] * Ytel[modes == "tel"])
...
results %>% 
  ...
  group_by(MBtype, sampling, YtelLaw, YintLaw, phi, estimator, probSelect, invMatrix) %>% 
  summarise(reldiffMean = round(mean(diff / abs(expectedPhiBias)) , 4L), 
            relRMSE = round(sqrt(mean(diff^2L / expectedPhiBias^2L)), 4L)) %>% 
  ungroup() %>% 
  group_by(MBtype, YtelLaw, YintLaw, sampling, phi) %>% 
  slice_min(relRMSE) # %>% 
  # knitr::kable()
```
