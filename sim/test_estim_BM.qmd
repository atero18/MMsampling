...
pi <- rep(n / N, N)
  ...
  diag(piMat) <- n / N
expVarSr <- f * sum(pInt * (1.0 - pInt)) + t(pInt) %*%  %*% pInt
```
                            ...
                            seed = 123L)
{
  ...
  with_progress({res <- lapply(seq_len(K), FUN = monoSim)})
...
for (delt in deltas)
{
  ...
  absRes <- rbind(absRes, count_superposition(data1, data2))
}

rm(data1, data2, delt)

absRes <- as_tibble(absRes) %>% 
...
for (delt in deltas)
{
  ...
  centeredRes <- rbind(centeredRes, count_superposition(data1, data2))
}

rm(data1, data2, delt)

centeredRes <- as_tibble(centeredRes) %>% 
  mutate(ratioVar1 = AV1 / var1, .after = "var1") %>% 
  mutate(ratioVar2 = AV2 / var2, .after = "var2")
