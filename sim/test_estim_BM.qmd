...
simulationUncounf <- 
           ...
           n = ceiling(N / 4L))
  {
    ...
    if (fixZ)
    {
      ...
      if (YintLaw == "gaussian")
      {
        expYint <- as.vector(Z %*% betaTel)
        covarYint <- diag(sdTel^2L, N)
      }
      else if (YintLaw == "exponential")
      {
        expYint <- as.vector(abs(Z %*% betaTel))
        covarYint <- diag(expYtel^2L)
      }
    ...
    monoSim <- function(...)
    {
      ...
      expPhiBias <- t(phi) %*% Z %*% (betaInt - betaTel)
      ...
      partialResults <- partialResults %>% 
        ...
        add_column(.after = "expPhiBias", estTotalPhiYintTrueMP = estTotalPhiYintHTTrueMP) %>% 
    ...
    if (fixZ)
    {
      ...
      for (l in seq_len(nrow(parameters)))
      {
        ...
        if (parameter$parameter == "HT" && parameter$invMatrix == "true" && parameter$calculTotal == "full")
        {
          ##browser()
          expSD <- var_estim_tot_BM(modeTotBiased = "HT", modeTotRef = "HT",
                                    calculTotal = "full",
...
results %>% 
  ...
  group_by(sampling, YintLaw, YtelLaw, MBtype, phi) %>% 
  summarise(expSD = mean(expSD), estSD = sd(estTotalYtelTrueMP))
```
...
results %>% 
  ...
  summarise(reldiffMean = round(mean(diffPhiBias / abs(expPhiBias)), 4L), 
...
results %>% 
  ...
  summarise(#reldiffTotalTrueMean = round(mean(diffTotalYtelTrueMP / abs(expTotalYtel)) , 4L), 
            reldiffTotalEstMean = round(mean(diffTotalYtelEstMP / abs(expTotalYtel)), 4L),
            ...
            relRMSEEst = round(sqrt(mean(diffTotalYtelEstMP^2L / expTotalYtel^2L)), 4L),
            relExpSD = round(relExpSD = mean(expSD / expTotalYtel), 4L),
            relRMSEBenchmark = round(sqrt(mean(diffBenchmark^2L / expTotalYtel^2L)), 4L),
            relSdTrueWeightsBenchmark = mean(sdTrueWeightsBenchmark / abs(expTotalYtel))) %>% 
  ungroup() %>% 
  arrange(relRMSEEst)
```
...
temp <- results %>% 
  ...
  summarise(#reldiffTotalTrueMean = round(mean(diffTotalYtelTrueMP / abs(expTotalYtel)) , 4L), 
            ...
            relRMSEEst = round(sqrt(mean(diffTotalYtelEstMP^2L / expTotalYtel^2L)), 4L),
            relRMSEBenchmark = round(sqrt(mean(diffBenchmark^2L / expTotalYtel^2L)), 4L),
            relSdBenchmark = mean(sdBenchmark / abs(expTotalYtel))) %>% 
  ungroup() %>% 
  arrange(relRMSEEst)
