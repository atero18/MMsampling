...
parameters <- parameters %>% 
  filter(sigma == sigmavec[1L] | !ratioDelta %in% c(0.1, 0.75)) %>% 
  mutate(delta = ratioDelta * abs(expMean2), .after = "ratioDelta")
...
-   \% of superposition of the confidence intervals with unbiased variance estimator for the second half part of the iterations, based on the previous estimation of the standard deviations

-   \% of superposition of the confidence intervals with approximate variance estimator

```{r}
count_superposition <- function(data1, data2, alpha1 = 0.05, alpha2 = alpha1)
{
...
```{r}
absRes %>% 
  group_by(weights, sigma, ratioDelta)
```
...
absRes %>% 
  ...
  filter(weights == "true") %>% 
  ggplot() +
  geom_point(aes(x = ratioDelta,
                 y = percSuperpositionsAV,
                 colour = sigma)) +
  # geom_smooth(aes(x = ratioDelta,
  #                y = percSuperpositionsAV, colour = sigma, n = 3)) +
  ggtitle("Percentage of superposition with true probabilities")
```
...
absRes %>% 
  ...
  # geom_smooth(aes(x = ratioDelta,
  #                y = percSuperpositionsAV, colour = sigma, n = 3)) +
  ggtitle("Percentage of superposition with estimated probabilities")
```
...
centeredRes %>% 
  ...
  # geom_smooth(aes(x = ratioDelta,
  #                y = percSuperpositionsAV, colour = sigma, n = 3)) +
  ggtitle("Percentage of superposition with mean and true probabilities")
```
...
centeredRes %>% 
  ...
  # geom_smooth(aes(x = ratioDelta,
  #                y = percSuperpositionsAV, colour = sigma, n = 3)) +
  ggtitle("Percentage of superposition with mean and estimated probabilities")
```
