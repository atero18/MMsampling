                            ...
                            seed = 123L)
{
  ...
  monoSim <- function(...)
  {
    ...
    totCtrdPhi1TW <- 
      sum(phi[maskInt] * 
            (trueWeights[maskInt] * Yobs[maskInt] - mean(Yints))) /
    ...
    totCtrdPhi2TW <- 
      sum(phi[maskTel] * 
            (trueWeights[maskTel] * Yobs[maskTel] - mean(Ytels))) /
...
count_superposition <- function(data1, data2, alpha1 = 0.05, alpha2 = 0.05)
{
  ...
  sd1 <- sd(data1[subsetCalcVar, "estimation"])
  sd2 <- sd(data2[subsetCalcVar, "estimation"])
  
  # We only keep the second half of the results for
  # other estimations.
  
  # Estimation of the approximate variance
  AV1 <- mean(data1[-subsetCalcVar, "apprVar"])
  AV2 <- mean(data2[-subsetCalcVar, "apprVar"])
  
  # Confidence intervals with unbiased estimator for the variance
  #   first mode
  CI1lowerUV <- data1[-subsetCalcVar, "estimation"] - 
    qnorm(1.0 - alpha1 / 2.0) * sd1
  CI1upperUV <- data1[-subsetCalcVar, "estimation"] + 
    qnorm(1.0 - alpha1 / 2.0) * sd1
  
  #   second mode
  CI2lowerUV <- data2[-subsetCalcVar, "estimation"] - 
    qnorm(1.0 - alpha2 / 2.0) * sd2
  CI2upperUV <- data2[-subsetCalcVar, "estimation"] + 
    qnorm(1.0 - alpha2 / 2.0) * sd2
  ...
  # Confidence intervals with approximate variance for each iteration
  #   first mode
  CI1lowerAV <- data1$estimation - 
    qnorm(1.0 - alpha1 / 2.0) * data1$apprVar
  CI1upperAV <- data1$estimation + 
    qnorm(1.0 - alpha1 / 2.0) * data1$apprVar
  
  #   second mode
  CI2lowerAV <- data2$estimation - 
    qnorm(1.0 - alpha2 / 2.0) * data2$apprVar
