...
simulationUncounf <- 
           ...
           n = ceiling(N / 4L))
  {
    ...
    nbExperiments <- 6L
    emptyResults <- data.frame(trueTotalparameter = character(nbExperiments),
    ...
    for (k in seq_len(K))
    {
      ...
      if (YintLaw == "gaussian")
        Yint <- Z %*% betaInt + rnorm(N, 0.0, sdInt)
      ...
      partialResults[row,] <- c(parameter = "G-COMP", trueParameter = TRUE,
                         probSelect = "null", 
      ...
      partialResults[row,] <- c(parameter = "MCO_unique",
                         trueParameter = TRUE,
      ...
      partialResults[row,] <- c(parameter = "HT", trueParameter = TRUE,
...
```{r}
temp <- results %>% 
  filter(trueEstimator, MBtype == "variable") %>% 
