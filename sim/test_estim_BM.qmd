                            ...
                            calcVariance = TRUE)
{
  ...
  monoSim <- function(...)
  {
    ...
    # Estimations with Y1
     ...
     # Total estimation of phi_k y_1k with true probabilities
    totPhi1TW <- sum(trueWeights[maskSr] * phi[maskSr] * Yobs[maskSr])
    ...
    {
      apprVarPhi1TW <-
        MMsampling::estim_AV_seq_phi1(Yobs,
                                            modes,
                                            I,
                                            pi_mat,
                                            p1,
    ...
    totPhi1EW <- sum(phi[maskSr] * estimWeights[maskSr] * Yobs[maskSr])
    # totPhi1EW <- NA_real_
    ...
    meanPhi1TW <- totPhi1TW / sum(phi[maskSr] * trueWeights[maskSr])
    ...
    meanPhi1EW <- totPhi1EW / sum(phi[maskSr] * estimWeights[maskSr])
    ...
    totPhi2TW <- sum(phi[maskSmr] * trueWeights[maskSmr] * Yobs[maskSmr])
    ...
    {
      apprVarPhi2TW <-
        MMsampling::estim_AV_seq_phi2(Yobs,
                                            modes,
                                            I,
                                            pi_mat,
                                            p1,
    ...
    totPhi2EW <- sum(phi[maskSmr] * estimWeights[maskSmr] * Yobs[maskSmr])
    # totPhi2EW <- NA_real_
    ...
    meanPhi2TW <- totPhi2TW / sum(phi[maskSmr] * trueWeights[maskSmr])
    # meanPhi2TW <- NA_real_
    ...
    meanPhi2EW <- totPhi2EW / sum(phi[maskSmr] * estimWeights[maskSmr])
    # meanPhi2EW <- NA_real_
  ...
  trueVarPhi1TW <- var_expansion_seq_phi1(Y1exp, pi_mat, p1, sd1, phi)
  
  trueVarPhi2TW <- var_expansion_seq_phi2(Y2exp, pi_mat, p1, p2, sd2, phi)
  ...
  res <- res %>% mutate(trueVar = trueVar, .after = "apprVar")
...
for (delta in deltas)
{
  ...
  .5 * avgEstWeightsInt * n1 * (delta - 1.0) - 
  estimatorsPhi2)
...
simulationUncounf <- 
           ...
           cluster = NULL)
  {
    ...
    clusterExport(cluster, c("gen_choice_bimode", 
                  "gen_Yexp", "gen_Y", "set_Yobs", "set_phi"))
    ...
    monoSim <- function(...)
    {
      ...
      #   trueProbsSelect <- dataModes$probSelect
      #   
      #   if (rankMatrix(Z[maskSr, , drop = FALSE]) == q &&
      #       rankMatrix(Z[maskSmr, , drop = FALSE]) == q)
      #     modeChoiceOK <- TRUE
      #   else
      ...
      # Creating the observed outcome vector
      # Yobs <- rep(NA_real_, N)
      # Yobs[maskSr] <- Yints[maskSr]
      # Yobs[maskSmr] <- Ytels[maskSmr]
      ...
      resEvalHT <- 
        as.vector(crossprod(phi[maskSr],
                            estimWeights[maskSr]) * 
                    deltaEval)
      ...
      resEvalHT <- crossprod(phi[maskSr] * estimWeights[maskSr],
                             estMBs[maskSr]) %>% as.vector()
      # toc() %>% print()
      ...
      benchmark <- crossprod(estimWeights[maskSmr], Ytels[maskSmr]) %>% 
        as.vector()
      trueWeightsBenchmark <- crossprod(trueWeights[maskSmr], Ytels[maskSmr]) %>% 
        as.vector()
      ...
      partialResults <- partialResults %>% 
        ...
        add_column(.after = "n", n1 = n1) %>% 
        add_column(.after = "n1", n2 = n2) %>% 
        add_column(.after = "n2", expYint = expTotYint) %>% 
        ...
        add_column(.after = "estPhiYintTrueMP", avgEstWeightsInt = mean(trueWeights[maskSr])) %>% 
        add_column(.after = "estPhiYintTrueMP", 
                   estPhiYintEstMP = estTotPhiYintHTEstMP) %>% 
        add_column(.after = "estPhiYintEstMP", avgEstWeightsTel = mean(trueWeights[maskSmr])) %>%
