...
$$\frac{1}{||\phi||_1}(||\phi||_1\delta_0 + \sum_{k \in U} \phi_k. age_k. \delta_{age} + \sum_{k \in U} \phi_k .sex_k. \delta_{sex})$$

In our situation we have that $\sum_{k \in U} age_k. \delta_{age} + \sum_{k \in U}.sex_k. \delta_{sex} = 0$, by construction. So if the $\phi_k$ are constant (which is the case here), the expected value simply becomes $\delta_0$, which is equal to $1$.
