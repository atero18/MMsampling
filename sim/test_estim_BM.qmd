    ...
    monoSim <- function(...)
    {
      ...
      partialResults[row,] <- 
        c(parameter = "HT", trueEstimator = FALSE,
          probSelect = "true", invMatrix = "population",
      ...
      partialResults[c(row, row + 1L),] <- 
        data.frame(parameter = "G-COMP_1_deg", trueEstimator = TRUE,
                   probSelect = "null", 
                   invMatrix = "null", calculTotal = c("population", "sample"),
                   estPhiBias = c(resEvalFull, resEvalHT))
      ...
      partialResults[c(row, row + 1L),] <- 
        data.frame(parameter = "HT", trueEstimator = TRUE,
                   probSelect = "estimation", invMatrix = "population", 
                   calculTotal = c("population", "sample"), 
                   estPhiBias = c(resEvalFull, resEvalHT))
      ...
      partialResults <- partialResults %>% 
        ...
        add_column(.after = "trueYtel", expYtel = expYtel) %>% 
    ...
    if (fixZ)
    {
      ...
      for (l in seq_len(nrow(parameters)))
      {
        ...
        if (parameter$parameter == "HT" && 
            ...
            parameter$invMatrix == "population" && 
...
results <- results %>% 
  ...
  mutate(.after = "estTotPhiYtelEstMP", 
         diffTotYtelEstMP = estYtelEstMP - trueYtel) %>% 
  mutate(.after = "estTotPhiYtelTrueMP",
         diffTotYtelTrueMP = estYtelTrueMP - trueYtel) %>% 
  mutate(.after = diffTotYtelTrueMP, diffBenchmark = benchmark - trueYtel)
```
...
temp <- results %>% 
  filter(sampling == "SRS",
         ...
         invMatrix == "population", calculTotal == "population",
...
We do not observe a clear stabilization of the variances.
...
The relative error `relError` is close to zero for each estimator. RMSEs are about the same, being in \[0.13, 0.18\]. For the best RMSE we have the model that consider the constant bias : the G-COMP model with a simple constant $\delta$ for the MB consideration.
...
temp %>% 
  summarise(K = n(),
            #reldiffTotalTrueMean = round(mean(diffTotYtelTrueMP / abs(expYtel)) , 4L), 
            relError = round(mean(diffTotYtelEstMP / abs(expYtel)), 4L),
            relBenchmarkError = round(mean(diffBenchmark / abs(expYtel)), 4L),
            #relRMSETrue = round(sqrt(mean(diffTotYtelTrueMP^2L / expYtel^2L)), 4L),
            relEstRMSE = round(sqrt(mean(diffTotYtelEstMP^2L / expYtel^2L)), 4L),
            #relExpTWSD = mean(sqrt(expVar2) / expYtel),
            relRMSEBenchmark = round(sqrt(mean(diffBenchmark^2L / expYtel^2L)), 4L),
            relExpSDTWBenchmark = mean(expSDTWBenchmark / abs(expYtel))) %>% 
  ungroup() %>% 
  arrange(relEstRMSE)
...
The `relRMSEBenchmark` variable is the RMSE of the benchmark estimator. We can observe that among the different estimators, we have a few of them that offer a lower RMSE than the benchmark. We have, with $\phi \equiv \frac{1}{2}$ :

-   G-COMPUTATION with 0, 1 or 2 degrees and a estimation of the total $\phi\Delta$ via the sample or the population

-   The HT Thompson estimator with a sample inverse matrix and a total on the sample or the population
...
The `relRMSEBenchmark` variable is the RMSE of the benchmark estimator

The `relExpSDTWBenchmark` variable is the expected standard deviation of the benchmark calculated with the true selection probabilities. Biases are nearly zero so RMSE and standard deviations can be compared. We can observe that for every estimator the (estimated) RMSE is clearly lower than the standard deviation of the benchmark with true probabilities.
...
temp <- results %>% 
  ...
  summarise(#reldiffTotalTrueMean = round(mean(diffTotYtelTrueMP / abs(expYtel)) , 4L), 
            relError = round(mean(diffTotYtelEstMP / abs(expYtel)) , 4L),
            relErrorBenchmark = round(mean(diffBenchmark / abs(expYtel)), 4L),
            #relRMSETrue = round(sqrt(mean(diffTotYtelTrueMP^2L / expYtel^2L)), 4L),
            relRMSEParam = round(sqrt(mean(diffTotYtelEstMP^2L / expYtel^2L)), 4L),
            relRMSEBenchmark = round(sqrt(mean(diffBenchmark^2L / expYtel^2L)), 4L),
            relExpSDTWBenchmark = round(mean(expSDTWBenchmark / abs(expYtel)), 4L)) %>% 
  ungroup() %>% 
  group_by(sd) %>% 
...
temp %>% 
  select(-relRMSEParam, -relRMSEBenchmark, -relExpSDTWBenchmark)
```

We can observe that the estimated bias is close to zero. It slightly increases with the conditional variance of $Y_{tel}$ and $Y_{int}$ (`sd`).

Even with a variable measure bias, the G-COMPT model with only a constant for the bias is approximately unbiased. It is due to the fact that we chose here a centered measure bias, respectively to the couple (age, sex).

And now the RMSE:

```{r}
temp %>% 
  select(-relError, -relErrorBenchmark, -probSelect) %>% 
  group_by(sd) %>% 
  arrange(relRMSEParam, .by_group = TRUE)
```

It is better to have $\phi_k \equiv \phi$ compared to $\phi_k = \frac{k}{N}$ for any conditional variance of $Y_{tel}$ and $Y_{int}$.

The estimator HT with complete inverse matrix and summing the bias on the sample only is the greatest (in term of RMSE) for any value and the standard deviation. Its RMSE is better than the RMSE of the benchmark, which is itself better than the benchmark with true probabilities.
...
Only a few of the estimators give an interesting RMSE. Some of the others can gives a RMSE about more than twice the RMSE of the benchmark. It is important to chose carefully the used estimator. On the other hand we can observe that the positive gain **is not really high** :
...
temp %>% 
  ...
  group_by(sd) %>% 
  summarize(minRatioRMSE = min(relRMSEParam / relRMSEBenchmark),
            maxRatioRMSE = max(relRMSEParam / relRMSEBenchmark))
```
...
```{r}
temp %>% 
  select(-relError, -relDiffBenchmarkMean, -probSelect) %>% 
  filter(relRMSEParam <= relRMSEBenchmark) %>% 
  group_by(phi, parameter, probSelect, invMatrix, calculTotal) %>% 
  summarize(n = n()) %>% 
  arrange(-n)
...
temp %>% 
  summarise(relError = 
              round(mean(diffTotYtelEstMP / abs(expYtel)), 4L))
```
...
RAJOUTER UN COMMENTAIRE Case when HT-population-population is the best:
...
Cases when the triples HT-sample-sample and HT-population-population are not optimal (or not the only one with the lowest RMSE):
...
À CHANGER : The both estimators seems to be more effective that the others when the measure bias is not a constant with random noise (they are not optimal in 100% of the constant scenarios).
