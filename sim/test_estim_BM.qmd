                            ...
                            nCores = detectCores() - 1L)
{
  ...
  res <- mclapply(seq_len(K), FUN = monoSim, mc.cores = nCores)
  do.call("rbind", res)
}
```
...
for (k in seq_along(ratioDeltas))
{
  ...
  resTemp <- simulation_tphi(K = Ksim,
                             Z = Z,
                             sampling = "SRS",
                             alphaInt = alphaInt,
                             alphaTel = alphaTel,
...
Here are the true variance $\hat{t}_{e\phi1}$, in the case of independency for both mode selections (intra & inter)
```{r}
...
Here we show average approximate variance and variance of different estimators for the first half of the iterations, with no measure effect.
```{r}
temp <- resSimulations %>% 
  filter(ratioDelta == 0.0)
...
```{r}
expData <- gen_expY(Z, betaInt = betaInt, sdInt = 1.0, betaTel = betaTel, sdTel = 1.0)
## Attention : expYtels ici
vartotphi1TW <- true_variance_tephi1(expData$expYtels, 
                                     pi, covar_SRS(n, N), 
                                     pInt,
                                     phi, 
                                     sd1 = 1.0)
vartotphi2TW <- true_variance_tephi2(expData$expYtels,
                                     pi, covar_SRS(n, N), 
                                     pInt, pTel, 
                                     phi, 
                                     sd2 = 1.0)
...
temp %>% 
  ...
  select(-weights) %>% 
  add_column(varTW = c(vartotphi1TW, vartotphi2TW)) %>% 
