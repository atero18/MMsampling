...
```{r}
sumBiases <- cumsum(temp[, c("diffPhiBias", "diffTotalYtelEstMP")])

Kvec <- seq_len(K)

biases <- 
  data.frame(totPhiDelta =  sumBiases[, "diffPhiBias"] / Kvec,
             totYtel = sumBiases[, "diffTotalYtelEstMP"] / Kvec,
             K = Kvec)
...
And the RMSE:
```{r}
sumSqErrors <- cumsum(temp[, c("diffPhiBias", "diffTotalYtelEstMP")]^2L)

RMSEs <- 
  data.frame(totPhiDelta =  sumSqErrors[-1L, "diffPhiBias"] / Kvec[-1L],
             totYtel = sumSqErrors[-1L, "diffTotalYtelEstMP"] / Kvec[-1L],
             K = Kvec[-1L])
...
temp %>% 
  summarise(K = n(),
            ...
            relExpSDTWBenchmark = mean(expSDTWBenchmark / abs(expTotalYtel))) %>% 
  ungroup() %>% 
...
```{r}
temp %>% 
  select(-relTotalError, -relDiffBenchmarkMean, -probSelect) %>% 
  filter(relRMSEParam <= relRMSEBenchmark) %>% 
