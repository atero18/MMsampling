...
parameters <- 
  rbind(expand.grid(ratio_delta = ratioDeltas, ratio_n = ratiosn, sigma = sigmavec[1L]),
        expand.grid(ratio_delta = ratioDeltas, ratio_n = ratiosn[1L], sigma = sigmavec[-1L]))

parameters <- parameters %>% 
  filter(sigma == sigmavec[1L] | !ratio_delta %in% c(0.1, 0.75)) %>% 
...
```{r}
absRes %>% 
  group_by(weights, sigma, ratio_delta)
```
