...
parameters <- 
  rbind(expand.grid(ratioDelta = ratioDeltas, n = nvec, sigma = sigmavec[1L]),
        expand.grid(ratioDelta = ratioDeltas, n = nvec[1L], sigma = sigmavec[-1L]))

parameters <- parameters %>% 
  filter(sigma == 1.0 | !ratioDelta %in% c(0.1, 0.75)) %>% 
  mutate(delta = ratioDelta * expMean2, .after = "ratioDelta")
...
for (k in nrow(parameters))
{
  ...
  resTemp <- simulation_tphi(K = Ksim,
                             ...
                             YtelLaw = "gaussian",
                             phi = "eq",
                             sdInt = parameters[k, "sigma"],
                             sdTel = parameters[k, "sigma"],
                             n = parameters[k, "n"],
                             seed = NULL,
                             nCores = 20L)
...
temp <- temp %>% 
  ...
  group_by(estimand, weights, n, sigma) %>% 
...
varsTW <- NULL
for (k in seq.int(from = 1L, to = nrow(temp), by = 2L))
{
  delta <- temp[k, "delta"]
  betaIntTemp <- betaTel
  betaIntTemp[1L] <- betaIntTemp[1L] + delta
  ...
  n <- temp[k, "n"]
  vartotphi1TW <- true_variance_tephi1(expData$expYints, 
                                       pi, covar_SRS(n, N), 
                                       pInt,
                                       phi, 
                                       sd1 = sigma)
  ...
  varsTW <- c(varsTW, vartotphi1EW, vartotphi2EW)
}
