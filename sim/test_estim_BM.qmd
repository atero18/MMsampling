...
simulationUncounf <- 
           ...
           cluster = NULL)
  {
    ...
    {
      sdTel <- "null"
      expYtels <- sdTels <- as.vector(abs(Z %*% betaTel))
      covarYtel <- diag(sdTels^2L)
    }
    ...
    {
      sdInt <- "null"
      expYints <- sdInts <- as.vector(abs(Z %*% betaInt))
      covarYint <- diag(sdInts^2L)
    }
    ...
    if (YintLaw %in% c("gaussian", "split3") && sdInt == "split3")
    {
      quantilesInt <- quantile(expYints, probs = c(1.0 / 3.0, 2.0 / 3.0))
      sdInts <- rep(3.0, N)
...
deltas <- ratioDeltas * abs(expMean2 / N)
```
...
CI2lower <- estimatorsPhi2 - qnorm(1.0 - alpha2 / 2.0) * empSD2
CI2upper <- estimatorsPhi2 + qnorm(1.0 - alpha2 / 2.0) * empSD2
...
avgEstWeightsInt <- as.numeric(temp$avgEstWeightsInt)
for (delta in deltas)
{
  # We remove the actual bias (equal to 1)
  phi1 <- estimatorsPhi1 + .5 * avgEstWeightsInt * nInt * (delta - 1.0)
  empSD1Delta <- sd(phi1)
  CI1lowerBis <- phi1 - qnorm(1.0 - alpha1 / 2.0) * empSD1Delta
  CI1upperBis <- phi1 + qnorm(1.0 - alpha1 / 2.0) * empSD1Delta
  ...
  presenceExpMean2 <- 
    (CI1lowerBis <= expMean2 & expMean2 <= CI1upperBis) & 
    (CI2lower <= expMean2 & expMean2 <= CI2upper)
...
resSuperpositions$alpha12 <- 1.0 - resSuperpositions$percPresenceMean2 / 100.0
...
with $1-\alpha_{12}$ the probability of having a superposition on the supposed total $t_2$ of the two confidence intervals. 
