                               ...
                               ...)
{
  ...
  {
     correctedPhiYStrq1 <- piSa^-1L * 
       (p1Sa^-1L * phi[maskRespStr] * Y1Est[maskRespStr] - 
          p1barSa^-1L * phibar[maskRespStr] * Y2Est[maskRespStr])
     
     if (correcEW1)
     {
       correctedPhiYStrq1 <- 
         correctedPhiYStrq1 + Z[maskRespStr, , drop = FALSE] %*% estLinCoefq1
     }
      ...
      varq1Est <- sum(weightsq1 * correctedPhiYStrq1^2L)
  }
  ...
  if (!m1Only)
  {
    correctedPhiYStrq2 <- (piSmr * p1barSmr * p2Smr)^-1L * phibarY2Smr
    if (correcEW2)
    {
      ...
      correctedPhiYStrq2 <- correctedPhiYStrq2 + ZSmr %*% estbPhibar22
    }
    
    varq2Est <- sum((1.0 - p2Smr) * correctedPhiYStrq2^2L)
  }
  else
    varq2Est <- 0.0
                                 ...
                                 strata = rep(1L, length(Yobs)))
{
  ...
  weightedYSr <- Yobs[maskSr] / pi[maskSr]

  weightedYSmr <- Yobs[maskSmr] / (pi[maskSmr] * p2[maskSmr])
  ...
  # SRS
  varpEst <- (covpklStr / piklStr) * # sum(tcrossprod(weightedYSr)) +
    sum(weightedYSr %*% t(weightedYSr)) + 
    # (covpklStr / piklStr) * sum(tcrossprod(weightedYSmr)) + 
    sum(weightedYSmr %*% t(weightedYSmr)) +
    sum(((1.0 - samplingFractionStr) - (covpklStr / piklStr)) * weightedYSr^2L) +
    sum(((1.0 - samplingFractionStr) * p2Smr - (covpklStr / piklStr)) * weightedYSmr^2L)
...
for (chunk in seq_len(Mtotal / Mchunk))
{
  ...
  aws.s3::s3write_using(
    resSimulation,
    FUN = saveRDS,
    object = glue("2025_07_01_{Mchunk}_1_to_{chunk}.rds"),
  ...
  aws.s3::s3write_using(
    seedEndChunk,
    FUN = write,
    object = glue("2025_07_01_{Mchunk}_seed_end_{chunk}.txt"),
    bucket = "ttassin/multimode/article_no_ME",
    opts = list("region" = "")
  )
}
...
aws.s3::s3write_using(
    resSimulation,
    FUN = saveRDS,
    object = glue("2025_07_01_10000.rds"),
    bucket = "ttassin/multimode/article_no_ME",
    opts = list("region" = "")
  )
