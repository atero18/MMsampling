                               ...
                               ...)
{
  ...
  covpkl <- -samplingFraction * (1.0 - samplingFraction) / (N - 1L) 
  if (m1Only)
  {
    ...
    varpEst <-
      ...
      (covpkl / pikl) * # sum(tcrossprod(correctedY1Srp))
      correctedY1Srp %*%
      t(correctedY1Srp) %>%
      sum()
    varpEst <- varpEst +
      sum(((1.0 - samplingFraction) / (samplingFraction^2L * p1Sr) - 
             covpkl / (pikl * samplingFraction^2L * p1Sr^2L)) *
            Y1Sr^2L)
  ...
  else if (m2Only)
  {
    ...
    # SRS
    varpEst <-
      (covpkl / pikl) * # sum(tcrossprod(correctedY2Smrp))
      correctedY2Smrp %*%
      t(correctedY2Smrp) %>%
      sum()
    varpEst <- varpEst +
      sum(((1.0 - samplingFraction) / 
             (samplingFraction^2L * p1barSmr * p2Smr) -
             covpkl / (samplingFraction^2L * pikl * p1barSmr^2L * p2Smr^2L)) *
            Y2Smr^2L)
  ...
  else
  {
    ...
    # SRS
    varpSrEst <-
      (covpkl / pikl) * # sum(tcrossprod(correctedPhiYSrp))
      correctedPhiYSrp %*%
      t(correctedPhiYSrp) %>%
      sum()
    varpSrEst <- varpSrEst + 
    ...
    # SRS
    varpSmrEst <-
      (covpkl / pikl) * # sum(tcrossprod(correctedPhiYSmrp))
      correctedPhiYSmrp %*%
      t(correctedPhiYSmrp) %>%
      sum()
    varpSmrEst <- varpSmrEst +
                                 ...
                                 covarp = NULL)
{
  ...
  # SRS
  varpEst <- (covpkl / pikl) * # sum(tcrossprod(weightedYSr)) +
    sum(weightedYSr %*% t(weightedYSr)) + 
    # (covpkl / pikl) * sum(tcrossprod(weightedYSmr)) + 
    sum(weightedYSmr %*% t(weightedYSmr)) +
    sum(((1.0 - samplingFraction) - (covpkl / pikl)) * weightedYSr^2L) +
    sum(((1.0 - samplingFraction) * p2Smr - (covpkl / pikl)) * weightedYSmr^2L)
                                ...
                                phi = rep(0.5, length(Y1exp)))
{
  ...
  samplingFraction <- pi[1L]
  covpkl <- -samplingFraction * (1.0 - samplingFraction) / (N - 1L)
  varp <- covpkl * sum(correctedYp %*% t(correctedYp)) +
    sum(samplingFraction^-2L * (samplingFraction * (1.0 - samplingFraction) - covpkl))
