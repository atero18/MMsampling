                            ...
                            calcVariance = TRUE)
{
  ...
  monoSim <- function(...)
  {
    ...
    # Estimations with Y1
     # Total estimation of y_1k  with true probabilities
    ...
    if (calcVariance)
    {
      ...
      # Total of y_1k with estimated probabilities
    ...
    # Estimations with Y2
      # Total of y_2k with true probabilities
    ...
    estimp2 <-  estimProbsData$conditional[, "m2"]
      ...
      # Total of y_2k with estimated probabilities
    ...
    # Estimations with Y1 and Y2 consider equal
    
    if (sd1 == sd2)
    {
      YobsEq <- Yobs
      YobsEq[modes == "m2"] <- Y1[modes == "m2"]
    ...
    tibble(estimator = c("tot1TW", "tot1EW", "tot2TW", "tot2EW"), 
           estimation = c(tot1TW, tot1EW, tot2TW, tot2EW),
  ...
  trueVarPhi1TW <- var_expansion_m1(Y1exp, pi_mat, p1, phi, sd1)
  
  trueVarPhi2TW <- var_expansion_m2(Y2exp, pi_mat, p1, p2, phi, sd2)
...
for (k in seq_len(nrow(temp) / 2L))
{
  ...
  n <- temp[k, "n"]
  vartot1TW <- true_variance_tephi1(Y1exp_list[[as.character(delta)]], 
                                    pi, covarSRS, 
                                    p1,
                                    phi, 
                                    sd1 = sigma)
