                                     ...
                                     Z = matrix(1.0, nrow = len(Yobs), ncol = 1L))
{
  ...
  maskm1Sa <- modes[maskSa] == "m1"
  
  m1Only <- all(phi[maskSmr] == 1.0)
  m2Only <- all(phi[maskSr] == 0.0)
  ...
  else if (m2Only)
  {
    ...
    Y2Smr <- Yobs[maskSmr]
    correctedY2Smrp <- (piSmr * p1Smrbar * p2Smr)^-1L * Y2Smr
    varpEst <-
      t(correctedY2Smrp) %*%
      (covarpSmr / piSmr_mat) %*%
      correctedY2Smrp %>%
    ...
    varpEst <- varpEst +
      sum((1.0 - piSmr) * piSmr^-2L * (p1Smrbar * p2Smr)^-1L *
            (1.0 - (p1Smrbar * p2Smr)^-1L) * Y2Smr^2L)
  ...
  else
  {
    ...
    Y1SaEst <- Y2SaEst <- Yobs[maskSa]
    

    phiY1SaEst <- phi[maskSa] * Y1SaEst
    phiY2SaEst <- (1.0 - phi[maskSa]) * Y2SaEst
    
    correctedPhiYSap <- phiY1SaEst + phiY2SaEst
    correctedPhiYSap[maskm1Sa] <-  * correctedPhiYSap[maskm1Sa]
