...
piMat_SRS <- memoise::memoise(piMat_SRS)
```
...
```{r}
alphaa <- solve(t(Z) %*% Z) %*% colSums(logit(pRep) * Z)
pa <- gen_selection_probabilities(Z, alphaa)
```
...
Notes 02/07/2025: Y %*% Y^T seems to be faster than tcrossprod(Y) (3-5 seconds for 10 iterations in sequential).
                               ...
                               ...)
{
  ...
  if (!m2Only)
  {
    ...
    # else
    #   covarpSr <- covarp[maskSr, maskSr, drop = FALSE]
  }
  else
    sd1 <- 0.0
  ...
  if (!m1Only)
  {
    ...
    # else
    #   covarpSmr <- covarp[maskSmr, maskSmr, drop = FALSE]
  }
  else
  {
    sd2 <- 0.0
  ...
  # The variance will be calculated stratum by stratum.
  varpEst <- 0.0
  # The estimation of the counterfactuals (if needed) is made on the
  # entire population
  ...
  {
    NStr <- sum(strata == stratum)
    maskSampledStratum <- strata == stratum & I
    nStr <- sum(maskSampledStratum)
    
    piklStr <- (nStr * (nStr - 1L)) / (NStr * (NStr - 1L))
    samplingFractionStr <- nStr / NStr
    
    covpklStr <- -samplingFractionStr * (1.0 - samplingFractionStr) /
      (NStr - 1L)
    
    maskResp1Str <- strata == stratum & modes == "m1" 
    maskResp2Str <- strata == stratum & modes == "m2"
    maskRespStr <- maskResp1Str | maskResp2Str
    
    if (m1Only)
    {
      correctedY1Strp <- (samplingFractionStr * p1[maskResp1Str])^-1L * Yobs[maskResp1Str]
      # varpEst <-
      #   t(correctedY1Strp) %*%
      #   (covarpSr / piSr_mat) %*%
      #   correctedY1Strp %>%
      ...
      # SRS
      # varpEst <-
      #   (covpkl / piklStr) * # sum(tcrossprod(correctedY1Strp))
      #   correctedY1Strp %*%
      #   t(correctedY1Strp) %>%
      #   sum()
      # varpEst <- varpEst +
      #   sum(((1.0 - samplingFractionStr) / (samplingFractionStr^2L * p1Sr) - 
      #          covpkl / (piklStr * samplingFractionStr^2L * p1Sr^2L)) *
      #         Y1Sr^2L)
      
      # (ST)SRS
      varpEst <- varpEst + (covpklStr / piklStr) * # sum(tcrossprod(correctedY1Strp))
        correctedY1Strp %*%
        t(correctedY1Strp) %>%
        sum()
      varpEst <- varpEst +
        sum(((1.0 - samplingFractionStr) / (samplingFractionStr^2L * p1[maskResp1Str]) - 
               covpklStr / (piklStr * samplingFractionStr^2L * p1[maskResp1Str]^2L)) *
              Yobs[maskResp1Str]^2L)
    ...
    else if (m2Only)
    {
      ...
      correctedY2Strp <- 
        (samplingFractionStr * (1.0 - p1[maskResp2Str]) * p2[maskResp2Str])^-1L * Yobs[maskResp2Str]
      # varpEst <-
      #   t(correctedY2Strp) %*%
      #   (covarpSmr / piSmr_mat) %*%
      #   correctedY2Strp %>%
      ...
      # SRS
      varpEst <- varpEst +
        (covpklStr / piklStr) * # sum(tcrossprod(correctedY2Strp))
        correctedY2Strp %*%
        t(correctedY2Strp) %>%
        sum()
      varpEst <- varpEst +
        sum(((1.0 - samplingFractionStr) / 
               (samplingFractionStr^2L * (1.0 - p1[maskResp2Str]) * p2[maskResp2Str]) -
               covpklStr / (samplingFractionStr^2L * piklStr * (1.0 - p1[maskResp2Str])^2L * p2[maskResp2Str]^2L)) *
              Yobs[maskResp2Str]^2L)
    ...
    else
    {
      ...
      # the variance estimator based on Sr and the one based on Smr
        # m1 - stratum
      PhiY1m1Str <- phi[maskResp1Str] * Y1Est[maskResp1Str] + 
        phibar[maskResp1Str] * Y2Est[maskResp1Str]
      
      correctedPhiY1Strp <- 
        (samplingFractionStr * p1[maskResp1Str])^-1L * PhiY1m1Str
      ...
      # SRS
      varp1Est <- 
        (covpklStr / piklStr) * # sum(tcrossprod(phiY1Est1Str))
        correctedPhiY1Strp %*%
        t(correctedPhiY1Strp) %>%
        sum()
      varp1Est <- varp1Est + 
        sum(((1.0 - samplingFractionStr) / (samplingFractionStr^2L * p1[maskResp1Str]) - 
               covpklStr / (piklStr * samplingFractionStr^2L * p1[maskResp1Str]^2L)) *
        ...
        # m2 - stratum
      PhiYm2Str <- phi[maskResp2Str] * Y1Est[maskResp2Str] + 
        phibar[maskResp2Str] * Y2Est[maskResp2Str]
      
      correctedPhiY2Str <- 
        (samplingFractionStr * 
           (1.0 - p1[maskResp2Str]) * p2[maskResp2Str])^-1L *
        PhiYm2Str
      ...
      # varp2Est <- varp2Est +
      #   sum((1.0 - piSmr) * piSmr^-2L * (p1barSmr * p2Smr)^-1L *
      #         (1.0 - (p1barSmr * p2Smr)^-1L) * PhiYm2Str^2L)
      
      # SRS
      varp2Est <-
        (covpklStr / piklStr) * # sum(tcrossprod(correctedPhiY2Str))
        correctedPhiY2Str %*%
        t(correctedPhiY2Str) %>%
        sum()
      varp2Est <- varp2Est +
      ...
      varpEst <- varpEst + 
        weightSrEstVp * varp1Est + (1.0 - weightSrEstVp) * varp2Est
  ...
  if (m1Only)
  {
    correctedY1Srq1 <- (piSr * p1Sr)^-1L * Y1Sr
  ...
  {
     correctedPhiYq1 <- piSa^-1L * 
       (p1Sa^-1L * phi[maskSa] * Y1Est[maskSa] - 
          p1barSa^-1L * phibar[maskSa] * Y2Est[maskSa])
     
     if (correcEW1)
     {
       correctedPhiYq1 <- 
         correctedPhiYq1 + Z[maskSa, , drop = FALSE] %*% estLinCoefq1
     }
      ...
      varq1Est <- sum(weightsq1 * correctedPhiYq1^2L)
  }
  ...
  if (!m1Only)
  {
    correctedPhiY2 <- (piSmr * p1barSmr * p2Smr)^-1L * phibarY2Smr
    if (correcEW2)
    {
      ...
      correctedPhiY2 <- correctedPhiY2 + ZSmr %*% estbPhibar22
    }
    
    varq2Est <- sum((1.0 - p2Smr) * correctedPhiY2^2L)
  }
  else
    varq2Est <- 0.0
...
```{r}
#' Estimation of the variance of t_0s,e and the approximate variance 
#' of t_0s when Y1 = Y2
#' 
#' 
#' HOMOSCEDASTICITY, INDEPENDENCE
...
#' (symmetric numeric matrix of order N).
#' @param p2 vector containing the true or estimated probabilities p_2k.
#' Can be equal to NA for the units that didn't answer by m2
#' (numeric vector of size N).
#' @param sd true or estimated value of the standard deviation of 
#' the potential outcomes (positive scalar).
#' @param correcEW2 TRUE if we must apply a correction due to the
#' use of estimation of `p2` (boolean).
#' @param I vector containing the membership indicators equal to TRUE if unit k
#' has been selected. Used only if `correcEW2` is true 
#' (logical vector of size N).
#' @param Z design matrix. Used only when `correcEW2` is TRUE
#' (numeric matrix with N rows and q columns).
estim_AV_as_eq_no_p1 <- function(Yobs,
                                 modes,
                                 pi_mat,
                                 p2,
                                 sd = NULL,
                                 ...
                                 strata = rep(1L, length(Yobs)))
{
  ...
  for (stratum in unique(strata))
  {
    ...
    weightedY1Str <- Yobs[maskResp1Str] / pi[maskResp1Str]
    weightedY2Str <- Yobs[maskResp2Str] / (pi[maskResp2Str] * p2[maskResp2Str])
  
  # varpEst <- t(weightedYSr) %*% (covarpSr / piSr_mat) %*% weightedYSr + 
  #   t(weightedYSmr) %*% (covarpSmr / piSmr_mat) %*% weightedYSmr +
  #   sum((1.0 - piSmr) * weightedYSmr^2L * (p2Smr - 1.0))
  
  # SRS
  varpEst <- varpEst + 
    (covpklStr / piklStr) * # sum(tcrossprod(weightedYSr)) +
    sum(weightedY1Str %*% t(weightedY1Str)) + 
    # (covpklStr / piklStr) * sum(tcrossprod(weightedYSmr)) + 
    (covpklStr / piklStr) *
    sum(weightedY2Str %*% t(weightedY2Str)) +
    sum(((1.0 - samplingFractionStr) - (covpklStr / piklStr)) * weightedY1Str^2L) +
    sum(((1.0 - samplingFractionStr) * p2[maskResp2Str] - (covpklStr / piklStr)) * weightedY2Str^2L)
  }
  
  # m2 selection variability (q2, R2)
  weightedYSmr <- Yobs[maskSmr] / (pi[maskSmr] * p2Smr)
  if (correcEW2)
  {
    ZSmr <- Z[maskSmr, , drop = FALSE]
    estVs2 <- -crossprod(ZSmr, (1.0 - p2Smr) * weightedYSmr)

    maskSm <- I & !maskSr
    
    estbs2 <- 
      solve(Fisher_Information_Matrix(p2, Z, maskSm)) %*% estVs2
    
    weightedYSmr <- weightedYSmr + ZSmr %*% estbs2
  }
  
  varq2Est <- sum((1.0 - p2Smr) * weightedYSmr^2L)

  # Potential outcomes variability
  varYEst <- N * sd^2L
  
  varpEst + varq2Est + varYEst
}
```
...
```{r}
var_expansion_as_eq_no_p1 <- function(Yexp, 
                                      pi_mat, 
                                      p1, p2, 
                                      sd = 0.0)
{
  ...
  # Sampling design variability (p, S)
  weightedYexp <- Yexp / pi
  covarp <- pi2_to_covarInc(pi_mat)
  varp <- t(weightedYexp) %*% covarp %*% weightedYexp +
    sum(pi^-1L * (1.0 - pi)) * sd^2L
  
  ## SRS CASE
  # piklStr <- pi_mat[1L, 2L]
  # samplingFractionStr <- pi[1L]
  # covpklStr <- -samplingFractionStr * (1.0 - samplingFractionStr) / (N - 1L)
  # varp <- covpkl * samplingFractionStr^-2L * sum(Yexp %*% t(Yexp)) +
  #   samplingFractionStr^-1L * sum((Yexp^2L + sd^2L)) -
  #   samplingFractionStr^-2L * sum(Yexp^2L)
  
  # m2 selection variability (q2, R2)
  varq2 <- sum((pi * p2)^-1L * p1 * (1.0 - p2) * (Yexp^2L + sd^2L))
...
```{r}
# Generates the second order inclusion probabilities matrix of a
# STratified Simple Random Sampling (STSRS) design
piMat_STSRS <- function(n_vec, strata)
{
  Nstrata <- table(strata)
  pi <- n_vec[strata] / Nstrata[strata]
  pi_mat <- outer(pi, pi)
  
  for (stratum in seq_along(n))
  {
    nStr <- n_vec[stratum]
    NStr <- sum(strata == stratum)
    pi_mat[which(strata == stratum), 
           which(strata == stratum)] <- piMat_SRS(nStr, NStr)
  }
...
# Results are stored in memory
piMat_STSRS <- memoise::memoise(piMat_STSRS)
```
...
parameters <- tidyr::expand_grid(R_Pe = R_Sp_vec, 
                            ...
                            Y = NULL)
{
  ...
  # If no strata are given, we consider one unique stratum
  if (is.null(strata))
    strata <- rep(1L, N)
  ...
  monoSim <- function(...)
  {
    ...
    else
    {
      ...
      # With true probabilities
    totPhiTW <- estim_tot_phi_y(Yobs, modes, pi, p1, p2, phi)
    # Case phi = #S_r. / #S_a
    phiSize <- rep(sum(maskSr) / (sum(maskSr) + sum(maskSmr)), N)
    totPhiSizeTW <- estim_tot_phi_y(Yobs, modes, pi, p1, p2, phiSize)
    # Case phi = p1, which gives the estimator 
    # t^ = \sum_{k \in \Sr} y_{1k} / pi_k 
    #  + \sum_{k \in \Sr} y_{2k} / (pi_k p_2k)
    totNop1TW <- estim_tot_0_no_p1(Yobs, modes, pi, p2)
    
    if (calcVariance)
    {
      apprVarPhiTW <- 
        estim_AV_seq_m1_m2(Yobs,
                           modes,
                           pi_mat,
                           p1,
                           p2,
      ...
      apprVarNop1TW <- 
        estim_AV_as_eq_no_p1(Yobs,
                             modes,
                             pi_mat,
                             p2,
                             sd1, # Variance estimated with sd1 only
                             correcEW2 = FALSE,
                             I = I,
                             Z = Z,
                             covarp = NULL,
      ...
      # With estimated probabilities
    totPhiEW <- estim_tot_phi_y(Yobs, modes, pi, estimp1, estimp2, phi)
    totPhiSizeEW <- estim_tot_phi_y(Yobs, modes, pi, estimp1, estimp2, phiSize)
    # For phi = p1, we do not estimate the probabilities p1k
    totNop1EW <- estim_tot_0_no_p1(Yobs, modes, pi, estimp2)
    if (calcVariance)
    {
      apprVarPhiEW <- 
        estim_AV_seq_m1_m2(Yobs,
  ...
  # trueVar <- rep.int(c(NA_real_, NA_real_, 
  #                      NA_real_, NA_real_,
  #                      NA_real_, NA_real_,
  #                      NA_real_, NA_real_,
  #                      NA_real_, NA_real_,
...
nbChunks <- ceiling(Mtotal / Mchunk)
...
for (chunk in seq_len(Mtotal / Mchunk))
{
  for (k in seq_len(nrow(parameters)))
  {
    if (parameters[k, "R_Pe"] == 0.2)
      Y <- Y0.2
    else if (parameters[k, "R_Pe"] == 0.5)
      Y <- Y0.5
  ...
  aws.s3::s3write_using(
    resSimulation,
    FUN = saveRDS,
    object = glue("2025_07_07_{Mchunk}_1_to_{chunk}.rds"),
    bucket = "ttassin/multimode/article_no_ME",
    opts = list("region" = "")
  )
  
  seedEndChunk <- .Random.seed
  aws.s3::s3write_using(
    seedEndChunk,
    FUN = write,
    object = glue("2025_07_07_{Mchunk}_seed_end_{chunk}.txt"),
    bucket = "ttassin/multimode/article_no_ME",
    opts = list("region" = "")
  )
}

rm(pb, resTempWithVar, resTempWithoutVar, n, Y, seedEndChunk)
...
aws.s3::s3write_using(
    resSimulation,
    FUN = saveRDS,
    object = glue("2025_07_07_{Mtotal}.rds"),
    bucket = "ttassin/multimode/article_no_ME",
    opts = list("region" = "")
  )
...
temp <- resSimulation %>% 
  ...
  group_by(mechanism, R_Pe, samplingDesign, n,
           estimand, phi, sample) %>% 
  summarise(M = n() / 2L,
...
resSimulation %>% 
  ...
  group_by(R_Pe, samplingDesign, n, estimand, phi, weights) %>%
  summarise(MC_var = var(estimation, na.rm = TRUE)) %>% 
  group_by(R_Pe, samplingDesign, n) %>% 
...
temp <- resSimulation %>% 
  filter(subset == 2L, 
         ...
         weights == "estimated") %>% 
  group_by(R_Pe, samplingDesign, n, estimand, phi) %>%
  summarise(MC_var = var(estimation, na.rm = TRUE)) %>%
  group_by(R_Pe, samplingDesign, n) %>% 
