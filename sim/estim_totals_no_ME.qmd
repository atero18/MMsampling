...
```{r}
estim_cov_12 <- function(Y1, Y2, sd1, sd2, Z, Yobs = NULL, modes, ...)
{
...
for (k in seq_len(nrow(parameters)))
{
  ...
  resSimulation <-
    cbind(parameters[k, ], 
          ...
          row.names = NULL) %>%
    mutate(eq12 = parameters[k, "sd1"] == parameters[k, "sd2"] &&
             parameters[k, "rho"] == 1.0) %>% 
    rbind(resSimulation)
...
resSimulation %>% 
  filter(subset == 1L) %>% 
  group_by(n, sd1, sd2, rho, mechanism, estimand, sample, weights) %>% 
  summarize(M = n(),
