                               ...
                               ...)
{
  ...
  for (stratum in unique(strata))
  {
    ...
    samplingFractionStr <- nStratum / Nstratum
    ...
    if (m1Only)
    {
      ...
      #   sum()
      # varpEst <- varpEst +
      #   sum(((1.0 - samplingFraction) / (samplingFraction^2L * p1Sr) - 
      #          covpkl / (piklStr * samplingFraction^2L * p1Sr^2L)) *
      #         Y1Sr^2L)
    ...
    else if (m2Only)
    {
      ...
      correctedY2Strp <- 
        (pi[maskResp2Str] * (1.0 - p1[maskResp2Str]) * p2[maskResp2Str])^-1L * Y2Smr
      # varpEst <-
      #   t(correctedY2Strp) %*%
      #   (covarpSmr / piSmr_mat) %*%
      #   correctedY2Strp %>%
      ...
      varpEst <- varpEst +
        ...
        sum()
      varpEst <- varpEst +
        sum(((1.0 - samplingFraction) / 
               (samplingFraction^2L * (1.0 - p1[maskResp2Str]) * p2[maskResp2Str]) -
               covpklStr / (samplingFraction^2L * piklStr * (1.0 - p1[maskResp2Str])^2L * p2[maskResp2Str]^2L)) *
              Y2Str^2L)
    ...
    else
    {
      ...
      # SRS
      varpSrEst <- 
        (covpklStr / piklStr) * # sum(tcrossprod(correctedPhiYSrp))
        correctedPhiYSrp %*%
        t(correctedPhiYSrp) %>%
        sum()
      varpSrEst <- varpSrEst + 
        sum(((1.0 - samplingFraction) / (samplingFraction^2L * p1Sr) - 
               covpklStr / (piklStr * samplingFraction^2L * p1Sr^2L)) *
              PhiYSr^2L)
      ...
      # SRS
      varpSmrEst <-
        (covpklStr / piklStr) * # sum(tcrossprod(correctedPhiYSmrp))
        correctedPhiYSmrp %*%
        t(correctedPhiYSmrp) %>%
        sum()
      varpSmrEst <- varpSmrEst +
        sum(((1.0 - samplingFraction) / 
               (samplingFraction^2L * p1barSmr * p2[maskResp2Str]) -
               covpklStr / (samplingFraction^2L * piklStr * (1.0 - p1[maskResp2Str])^2L * p2[maskResp2Str]^2L)) *
              PhiYSmr^2L)
      
      varpEst <- varpEst + 
        weightSrEstVp * varpSrEst + (1.0 - weightSrEstVp) * varpSmrEst
                                 ...
                                 strata = rep(1L, length(Yobs)))
{
  ...
  samplingFraction <- pi[1L]
  covpklStr <- -samplingFraction * (1.0 - samplingFraction) / (N - 1L) 
  ...
  varpEst <- (covpklStr / piklStr) * # sum(tcrossprod(weightedYSr)) +
    ...
    sum(((1.0 - samplingFraction) - (covpklStr / piklStr)) * weightedYSr^2L) +
    sum(((1.0 - samplingFraction) * p2Smr - (covpklStr / piklStr)) * weightedYSmr^2L)
                                      ...
                                      sd = 0.0)
{
  ...
  # samplingFraction <- pi[1L]
  # covpklStr <- -samplingFraction * (1.0 - samplingFraction) / (N - 1L)
  # varp <- covpkl * samplingFraction^-2L * sum(Yexp %*% t(Yexp)) +
  #   samplingFraction^-1L * sum((Yexp^2L + sd^2L)) -
  #   samplingFraction^-2L * sum(Yexp^2L)
