...
```{r}
estim_tot_m1 <- function(Yobs, modes, pi, p1, phi = rep(1.0, len(Yobs)))
{
  maskSr <- modes == "m1"
...
```{r}
estim_tot_m2 <- function(Yobs, modes, pi, p1, p2, phi = rep(1.0, len(Yobs)))
{
  maskSmr <- modes == "m2"
...
```{r}
estim_tot_m1_m2 <- function(Yobs, modes, pi, p1, p2, phi)
{
...
```{r}
estim_tot_a <- function(Yobs, modes, pi, pa)
{
  modes[modes == "m2"] <- "m1"
  
  estim_tot_m1(Yobs, modes, pi, pa)
}
```
                                  ...
                                  Z = matrix(1.0, nrow = len(Yobs), ncol = 1L))
{
  ...
  if (correcEstimWeights)
  {
    ...
    estbPhi21 <- solve(Fisher_Information_Matrix(p1, Z, I)) %*% estVPhi11
                                 ...
                                 Z = matrix(1.0, nrow = len(Yobs), ncol = 1L))
{
  ...
  estim_appr_var_seq_m1(Yobs, modes, I, 
                        pi_mat, pa, sd, 
                        correcEstimWeights = correcEstimWeights,
                        Z = Z)
}
```
                                     ...
                                     ...)
{
  ...
  m2Only <- m2Only || all(phi == 0.0, na.rm = TRUE)
  ...
  else if (m2Only)
  {
    ...
    # We will need here have to estimate the potential outcomes
  ...
  else
  {
    ...
    dataMatchingSa <- as.data.frame(Z[maskSa, , drop = FALSE]) %>% 
      mutate(mode = as.integer(maskm1Sa))
    row.names(dataMatchingSa) <- seq_len(na)
    
    # Estimation of the y_1k on Smr
    if (!all(phi[maskSm] == 0.0))
    {
      matchingControl <- 
        MatchIt::matchit(mode ~ ., data = dataMatchingSa, estimand = "ATC", ...)
      
      Y1SaEst[!maskm1Sa] <- YobsSa[as.integer(matchingControl$match.matrix)]
      
      rm(matchingControl)
    }
    
    # Estimation of the y_2k on Sr
    if (!all(phi[maskSr] == 1.0))
    {
      matchingTreated <- 
        MatchIt::matchit(mode ~ ., data = dataMatchingSa, estimand = "ATT", ...)
    
      Y2SaEst[maskm1Sa] <- YobsSa[as.integer(matchingTreated$match.matrix)]
...
parameters <- rbind(paramVar_n, paramVar_sd, paramVar_rho)
                            ...
                            calcVariance = TRUE)
{
  ...
  # Probability to answer with any mode : equality true for any vector
  # iff R2 indepdendent of R1 conditionally to (Z, Y1, Y2, S) 
  pa <- p1 + (1.0 - p1) * p2
  ...
  monoSim <- function(...)
  {
    ...
    dataModes <-  MMsampling::gen_choice_bimode(I, p1, p2, Z)
    ...
    # Estimations with Y1 and Y2 considered equal (to some Y)
    # In the case of V[Y1 | Z] != V[Y2 | Z] the equality
    # is clearly not respected
    if (sd1 == sd2)
    {
      
      YobsEq <- Yobs
      YobsEq[modes %in% c("m1", "m2")] <- Y1[modes %in% c("m1", "m2")]
      
      # With true probabilities
      totaTW <- estim_tot_a(Yobs, modes, pi, pa)
      ...
      # With estimated probabilities
      totaEW <- estim_tot_a(Yobs, modes, pi, pa)
    ...
    {
      totaTW <- apprVaraTW <- NaN
      totaEW <- apprVaraEW <- NaN
    }
  ...
  trueVar1TW <- var_expansion_m1(Yexp, pi_mat, p1, phi, sd1)
  
  trueVar2TW <- var_expansion_m2(Yexp, pi_mat, p1, p2, phi, sd2)
  
  if (sd1 == sd2)
    trueVaraTW <- var_expansion_a(Yexp, pi_mat, p1, sd1)
  else
    trueVaraTW <- NaN
  ...
  res <- res %>% 
    mutate(expEstimand = Yexp, .after = "estimand") %>% 
...
for (k in seq_len(nrow(parameters)))
{
  ...
  resSimulation <-
    cbind(parameters[k, ], 
          ...
          row.names = NULL) %>%
    filter(!is.nan(estimation), 
           !estimator %in% c("totaTW", "totaEW") | rho == rho_ref) %>% 
    mutate(rho = if_else(estimator %in% c("totaTW", "totaEW"), 1.0, rho)) %>% 
    rbind(resSimulation)

  pb$tick()
}

rm(resTempWithVar, resTempWithoutVar, pb)
