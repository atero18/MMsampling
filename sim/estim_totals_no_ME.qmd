...
#' set of respondents (numeric vector of size N).
...
#' and between the m1 and m2 outcomes. We will have an unbiased an consistent
#' estimator if the measure effect y_1k - y_2k follow a linear model with z_k
#' and for some units both potential outcomes are known (which can be under
#' a sequential protocol). The standard deviations `sd1` and `sd2` must be 
#' the true values or consistent estimators as well. We must have at least 
#' q + 1 (q the number of covariates) units such that their potential outcomes 
#' are known or well estimated.
...
#' potential outcomes. Default to `sd1` * `sd2` (scalar).
#' @param phi vector containing weights for the total. Must be known on the
#' set of respondents (numeric vector of size N).
                                     ...
                                     ...)
{
  ...
  else
  {
    ...
    phiY2SaEst <- (1.0 - phi[maskSa]) * Y2Est
  ...
  else if (is.null(cov12))
    cov12 <- estim_cov_12(Y1Est, Y2Est, sd1, sd2, Z)
  
  varphiYEst <- 
    mean(phi^2L, na.rm = TRUE) * N * sd1^2L +
    2.0 * mean(phi * (1.0 - phi), na.rm = TRUE) * N * cov12 +
