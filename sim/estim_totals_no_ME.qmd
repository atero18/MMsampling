...
```{r}
estim_tot_a <- function(Yobs, modes, pi, pa, phi = rep(1.0, len(Yobs)))
{
  maskSa <- modes %in% c("m1", "m2")
  
  sum((pi[maskSa] * pa[maskSa])^-1L * phi[maskSa] * Yobs[maskSa])
}
```
                            ...
                            calcVariance = TRUE)
{
  ...
  # Variance matrix of the potential outcomes, for the vector
  # (Y1^T, Y2^T)^T of R^(2N)
    # Variance of each potential outcome (homoscedasticity)
  varY_mat <- diag(c(rep(sd1^2L, N), rep(sd2^2L, N)))
  rownames(varY_mat) <- colnames(varY_mat) <- 
    c(paste0("y1", seq_len(N)), paste0("y2", seq_len(N)))
    # Covariance between the potential outcomes of m1 and m2
    # (same interaction, only for y1k and y2k)
  subCovarm1m2_mat <- diag(rho * sd1 * sd2, N)
  varY_mat[seq(N + 1L, 2L * N), seq_len(N)] <- 
    varY_mat[seq_len(N), seq(N + 1L, 2L * N)] <- subCovarm1m2_mat
  rm(subCovarm1m2_mat)

  phi <- set_phi(phi, N)
  YPhiexp <- sum(phi * Yexp)
  ...
  monoSim <- function(...)
  {
    ...
    Y <- 
      mvtnorm::rmvnorm(n = 1L, mean = rep.int(Yexp, 2L), 
                       sigma = varY_mat,
                       checkSymmetry = FALSE)[1L, ]
    names(Y) <- rownames(varY_mat)

    
    # Creating the observed outcome vector
    Yobs <- rep(NA_real_, N)
    Yobs[modes == "m1"] <- Y[modes == "m1"]
    Yobs[modes == "m2"] <- Y[modes == "m2"]
    
    # Estimations with Y1
     # Total estimation of phi_k y_1k  with true probabilities
    totPhi1TW <- estim_tot_m1(Yobs, modes, pi, p1, phi)
    ...
    totPhi1EW <- estim_tot_m1(Yobs, modes, pi, estimp1, phi)
    ...
    totPhi2TW <- estim_tot_m2(Yobs, modes, pi, p1, p2, phi)
    ...
    totPhi2EW <- estim_tot_m2(Yobs, modes, pi, estimp1, estimp2, phi)
    # totPhi2EW <- NA_real_
    ...
    tibble(estimand = rep(c("phi1", "phi2"), each = 2L), 
           estimator = c("totPhi1TW", "totPhi1EW", "totPhi2TW", "totPhi2EW"), 
           estimation = c(totPhi1TW, totPhi1EW, totPhi2TW, totPhi2EW),
  ...
  trueVarPhi1TW <- var_expansion_m1(Y1exp, piMat, p1, phi, sd1)
  
  trueVarPhi2TW <- var_expansion_m2(Y2exp, piMat, p1, p2, phi, sd2)
  ...
  res <- res %>% 
    mutate(expEstimand = YPhiexp, .after = "estimand") %>% 
