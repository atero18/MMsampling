...
```{r}
estim_appr_var_seq_m1 <- purrr::partial(estim_appr_var_seq_m1_m2,
                                        phi = rep(1.0, length(Yobs)), 
                                        m1Only = TRUE,
                                        sd2 = 0.0,
                                        rho = 0.0)
...
```{r}
estim_appr_var_seq_m2 <- purr::partial(estim_appr_var_seq_m1_m2,
                                       phi = numeric(Yobs),
                                       m2Only = TRUE,
                                       sd1 = 0.0,
                                       rho = 0.0)
                                ...
                                phi = rep(0.5, length(Y1exp)))
{
  ...
  varq1 <- 
    sum(pi^-1L * p1 * p1Bar * 
    (varCorrectedDifference + 
       p1^-1L * phi * Y1exp - p1bar^-1L * phibar * Y2exp))
...
```{r}
var_expansion_m1 <- purrr::partial(var_expansion_m1_m2,
                                   Y2exp = numeric(length(Y1exp)), 
                                   p2 = NULL,
                                   sd2 = 0.0, rho = 0.0)
```

```{r}
var_expansion_m2 <- purrr::partial(var_expansion_m1_m2,
                                   Y1exp = numeric(length(Y2exp)),
                                   sd1 = 0.0, rho = 0.0) 
```
                            ...
                            calcVariance = TRUE)
{
  ...
  monoSim <- function(...)
  {
    ...
    tibble(estimator = c("tot1TW", "tot1EW", 
           ...
           estimand = rep(c(sum(Y1), sum(Y2), sum(Y1),
                            sum(phi * Y1 + (1.0 - phi) * Y2)), each = 2L),
