                                     ...
                                     ...)
{
  ...
  {
    # If Y1 = Y2 on U we can set Y1Est = Y2Est equal
...
Y0.2 <- Y0.2 - mean(Y0.2 - N)
Rsq(Z, beta, epsilon0.2)
epsilon0.5 <- rnorm(N, mean = 0.0, sd = 3.27)
Y0.5 <- Z %*% beta + epsilon0.5
Y0.5 <- Y0.5 - mean(Y0.5 - N)
Rsq(Z, beta, epsilon0.5)
rm(epsilon0.2, epsilon0.5)
```

```{r}
Rsq(Z, beta, rnorm(N, mean = 0.0, sd = 5.0))
```
...
parameters <- tidyr::expand_grid(samplingDesign = samplingDesign_vec, 
                                 n = n_vec,
                                 "R²" = Rsq_vec)
  ...
  monoSim <- function(...)
  {
    ...
    tibble(estimator = c("tot1TW", "tot1EW", 
           ...
           mechanism = rep(c("sequentialLogit", "sequentialLogit", 
                             "globalLogit", "sequentialLogit", 
                             "sequentialLogit", "sequentialLogit"), each = 2L),
...
{
  if (parameters[k, "R²"] == 0.2)
    Y <- Y0.2
  else if (parameters[k, "R²"] == 0.5)
    Y <- Y0.5
  ...
  else if (parameters[k, "samplingDesign"] == "STSRS")
  {
    ...
    n <- rep(ceiling(parameters[k, "n"] / nbStrata), nbStrata)
  }
  ...
  simFun <- function(calcVariance = TRUE)
  {
    simulation_tphi(M = Msim,
                    ...
                    beta = NULL,
                    phi = phi,
                    sd1 = 0.0, # parameters[k, "sd1"],
                    sd2 = 0.0, # parameters[k, "sd2"],
                    n = n,
                    strata = strata,
                    rho = 1.0, # parameters[k, "rho"],
                    seed = NULL,
                    nCores = 40L,
...
resSimulation %>% 
  filter(subset == 1L) %>% 
  group_by(sd1, sd2, rho, 
           equalOutcomes, mechanism, 
           samplingDesign, n, 
...
### Impact of the potential outcome variability

The impact is supposed be negligible when the sampling fraction $f := nN^{-1}$ is small.
...
resSimulation %>% 
  ...
  group_by(sd1, sd2, rho, 
           ...
           samplingDesign, n, 
           estimand, weights) %>% 
  summarize(MC_var = var(estimation, na.rm = TRUE)) %>%
  group_by(sd2, rho, samplingDesign, n, estimand, weights) %>% 
  summarise(min_SD1 = min(sd1),
            max_SD1 = max(sd1),
            ratio_SD1 = max(sd1) / min(sd1),
            ratio_MC_var = max(MC_var) / min(MC_var),
            ratio_MC_var_ratio_SD1 = ratio_MC_var / ratio_SD1) %>% 
  filter(ratio_SD1 != 1.0)
```
...
resSimulation %>% 
  ...
  group_by(sd1, sd2, rho, 
           ...
           estimand, weights) %>%  
  summarize(MC_var = var(estimation, na.rm = TRUE)) %>%
  group_by(sd1, rho, mechanism, samplingDesign, n, estimand, weights) %>% 
  summarise(min_SD2 = min(sd2),
            max_SD2 = max(sd2),
            ratio_SD2 = max(sd2) / min(sd2),
            ratio_MC_var = max(MC_var) / min(MC_var),
            ratio_MC_var_ratio_SD2 = ratio_MC_var / ratio_SD2) %>% 
  filter(ratio_SD2 != 1.0)
```

```{r}
resSimulation %>% 
  filter(subset == 2L, rho == 0.8, 
         ratio_sd == 1.0, 
         estimand %in% c("t_1", "t_2"), 
         weights == "estimated") %>% 
  mutate(sd = sd2) %>% 
  group_by(sd, rho, mechanism, samplingDesign, n, estimand) %>% 
  summarise(MC_var = var(estimation, na.rm = TRUE),
            ratio_MC_var = MC_var) %>% 
  ungroup() %>% 
  ggplot() + geom_point(aes(x = sd, y = MC_var, color = estimand))
...
```{r}
resSimulation %>% 
  filter(subset == 2L, mechanism == "sequentialLogit") %>% 
  group_by(sd1, sd2, rho, equalOutcomes, 
           samplingDesign, n, estimand, phi, sample, weights) %>%
  summarize(MC_var = var(estimation, na.rm = TRUE)) %>% 
  group_by(n, sd1, sd2, rho) %>% 
...
resSimulation %>% 
  filter(subset == 2L, mechanism == "sequentialLogit") %>% 
  filter(ratio_sd > 1.0) %>% 
  group_by(sd1, sd2, rho, n, samplingDesign, estimand, phi, weights) %>%
  summarize(MC_var = var(estimation, na.rm = TRUE)) %>% 
  group_by(sd1, sd2, rho, samplingDesign,n) %>% 
  mutate(ratio_MC_var = max(MC_var) / min(MC_var)) %>% 
...
resSimulation %>% 
  filter(subset == 2L, mechanism == "sequentialLogit") %>% 
  filter(ratio_sd < 1.0) %>% 
  group_by(sd1, sd2, rho, samplingDesign, n, estimand, phi, weights) %>%
  summarize(MC_var = var(estimation, na.rm = TRUE)) %>% 
  group_by(sd1, sd2, rho, samplingDesign, n) %>% 
  mutate(ratio_MC_var = max(MC_var) / min(MC_var)) %>% 
  arrange(MC_var, .by_group = TRUE)
...
```{r}
temp <- resSimulation %>% 
  filter(subset == 2L, mechanism == "sequentialLogit", weights == "estimated") %>% 
  group_by(sd1, ratio_sd, rho, samplingDesign, n, estimand) %>%
  summarize(MC_var = var(estimation, na.rm = TRUE)) %>%
  select(-n) %>% 
  group_by(sd1, ratio_sd, rho, samplingDesign) %>% 
  arrange(MC_var, .by_group = TRUE) %>% 
  slice(c(1L, n())) %>% 
  arrange(sd1, ratio_sd, rho, .by_group = TRUE) %>% 
  arrange(MC_var, .by_group = TRUE) %>% 
  ungroup()

compareVar <- temp %>% select(sd1, ratio_sd, rho) %>% slice(seq(1L, nrow(temp), 2L))
