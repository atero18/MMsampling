...
Random generation of potential outcomes vectors, considered fixed afeter, in order to have $R²$ close to 0.2 and 0.5, respectively.
```{r}
set.seed(123L)
...
parameters <- tidyr::expand_grid("R²" = Rsq_vec, n = n_vec)
  ...
  resSimulation <-
    ...
    mutate(equalOutcomes = 
             ifelse(parameters[k, "sd1"] == parameters[k, "sd2"] &&
                      (parameters[k, "sd1"] == 0.0 || parameters[k, "rho"] == 1.0), 
                    "yes", "no")) %>% 
    rbind(resSimulation)
...
rm(pb, resTempWithVar, resTempWithoutVar, Y)

resSimulation <-
  resSimulation %>%
  mutate(phi = case_when(estimand == "t_phiy" ~ constPhi,
                         estimand == "t_phiSizey" ~ "sizeSr.",
                         estimand == "t_nop1y" ~ "p1",
                         TRUE = NaN)) %>% 
...
resSimulation %>% 
  filter(subset == 2L) %>% 
  group_by(n, sd1, sd2, rho, equalOutcomes, estimand, weights) %>% 
  summarize(MC_var = var(estimation, na.rm = TRUE)) %>%
  group_by(n, sd2, rho, estimand, weights) %>% 
...
resSimulation %>% 
  ...
  group_by(n, sd1, sd2, rho, equalOutcomes, estimand, weights) %>% 
  summarize(MC_var = var(estimation, na.rm = TRUE)) %>%
  group_by(n, sd1, rho, estimand, weights) %>% 
...
resSimulation %>% 
  ...
  group_by(n, sd1, sd2, rho, equalOutcomes, estimand, phi, sample, weights) %>%
  summarize(MC_var = var(estimation, na.rm = TRUE)) %>% 
...
resSimulation %>% 
  filter(subset == 2L, mechanism == "sequentialLogit") %>% 
  filter(ratio_sd > 1.0) %>% 
  group_by(n, sd1, sd2, rho, estimand, phi, weights) %>%
  summarize(MC_var = var(estimation, na.rm = TRUE)) %>% 
  group_by(n, sd1, sd2, rho) %>% 
  mutate(ratio_MC_var = max(MC_var) / min(MC_var)) %>% 
...
temp <- resSimulation %>% 
  ...
  group_by(n, sd1, ratio_sd, rho, estimand) %>%
  summarize(MC_var = var(estimation, na.rm = TRUE)) %>%
  select(-n) %>% 
  group_by(sd1, ratio_sd, rho) %>% 
  arrange(MC_var, .by_group = TRUE) %>% 
  slice(c(1L, n())) %>% 
