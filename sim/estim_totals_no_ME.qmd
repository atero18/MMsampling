  ...
  else if (samplingDesign == "STSRS")
  {
    ...
    for (stratum in unique(strata))
    {
      ...
      pi_mat[which(strata == stratum), 
             which(strata == stratum), 
             drop = FALSE] <- piMat_SRS(nStratum, NStratum)
    }
  }
...
for (k in seq_len(nrow(parameters)))
{
  ...
  if (parameters[k, "samplingDesign"] == "SRS")
    ...
    n <- rep(parameters[k, "n"], length(unique(strata)))
...
resSimulation <-
  resSimulation %>%
  mutate(phi = case_when(estimand == "t_phiy" ~ as.character(constPhi),
